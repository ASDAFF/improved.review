    ...
    const DB_TYPE = 'MYSQL';
        ...
        Function Delete($ID)
        {
            ...
            if($ar = $rs->fetch())
            {
                ...
                //$connection->queryExecute("DELETE FROM improved_review_vote_to_review WHERE REVIEW_ID = ".$ar["ID"]);
                global $DB;
                $DB->query("DELETE FROM improved_review_vote_to_review WHERE REVIEW_ID = ".$ar["ID"]);
                $DB->query("DELETE FROM improved_review_rating2element WHERE ELEMENT_ID = ".$ar["ELEMENT_ID"]);
                ...
                //IMPROVED\Review\ReviewTable::delete(array("ID"=>$ar["ID"]));
                $DB->query("DELETE FROM improved_review WHERE ID = ".$ar["ID"]);
